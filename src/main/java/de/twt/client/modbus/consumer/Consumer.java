...
public class Consumer {
	...
	public void writeDataThread() {
		...
		OrchestrationResponseDTO orchestrationResponse = getServiceProvider(ConsumerModbusConstants.WRITE_MODBUS_DATA_SERVICE_DEFINITION);
		if (orchestrationResponse == null) {
			...
			logger.warn("No provider with service \"{}\" found during the orchestration", 
					ConsumerModbusConstants.WRITE_MODBUS_DATA_SERVICE_DEFINITION);
			return;
		}
	...
	public void readDataThread() {
		...
		OrchestrationResponseDTO orchestrationResponse = getServiceProvider(ConsumerModbusConstants.READ_MODBUS_DATA_SERVICE_DEFINITION);
		if (orchestrationResponse == null) {
			logger.warn("No orchestration response received");
			return;
		} else if (orchestrationResponse.getResponse().isEmpty()) {
			logger.warn("No provider with service \"{}\" found during the orchestration", 
					ConsumerModbusConstants.READ_MODBUS_DATA_SERVICE_DEFINITION);
			return;
		}
	...
	private void readDataFromSlaveAddress(OrchestrationResultDTO orchestrationResult) {
		...
		while (!stopReadingData) {
			...
			logger.debug("readDataFromSlaveAddress: start one consume");
	...
	private void readOneDataFromSlaveAddress(OrchestrationResultDTO orchestrationResult){
    	...
    	logger.debug(Utilities.toJson(response));
	...
	public void sendDataToOPCUA() {
		logger.debug("sendDataToOPCUA: start sending data to OPC-UA...");
		// get the service providers from the arrowhead core system (orchestration)
